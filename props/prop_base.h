...
#define CHECK_SUPPORTS_COLOR_CHANGE(N) \
	...
	MENU_OPTIONS hover_menu_mode = MAIN_MENU;
	...
	void SelectMenuOption() {
		...
		{
			scroll_menu_mode = hover_menu_mode;
		}
		else {
	...
	void PlayMenuSound(const char* trackpath) {
#ifdef ENABLE_AUDIO
		if (track_player_) {
			track_player_->Stop();
			track_player_.Free();
